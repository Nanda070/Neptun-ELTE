...
class SessionGuard {
  ...
  class _APIRequest{
    ...
    static Future<_TokenRefreshOutcome> _attemptTokenRefresh() async {
      try {
        if (SessionGuard.isAuthBlocked) {
          return _TokenRefreshOutcome.transientFailure;
        }
        final refreshToken = storage.DataCache.getRefreshToken();
        if (refreshToken == null || refreshToken.isEmpty) {
          return _TokenRefreshOutcome.authRejected;
        }
        ...
        final response =
            await postRequestRaw(refreshUrl, "{}", bearerToken: refreshToken);

        if (response.statusCode == 200) {
          final bodyJson = conv.jsonDecode(response.body);
          if (bodyJson["data"] != null &&
              bodyJson["data"]["accessToken"] != null) {
    ...
    /// Foreground maintenance: refresh JWT on a timer while [AppLifecycleState.resumed].
