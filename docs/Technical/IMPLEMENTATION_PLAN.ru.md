...
| **Релиз**            | Текущая маркетинговая версия **1.3.3** (`pubspec` **1.3.3+1**). Feature-line **3** = пункты плана **1–3**; патч **3** = фикс мгновенного «сессия истекла» после 2FA. Следующий крупный блок → **1.4.0**; финал → **2.0.0**. Для пользователя / Settings / docs — только три числа, без рекламы `+build`. См. [TECHNICAL § Версионирование](TECHNICAL.ru.md#версионирование). |
...
| Settings | … существующие переключатели; **Contacts** + маркетинговая версия (`1.3.3`, без `+build`) внизу |
