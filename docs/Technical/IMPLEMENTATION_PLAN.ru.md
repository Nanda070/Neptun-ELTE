...
|                      |                                                                                                                                                                                                                                |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Статус**           | Foundation **1a / 1b / 1c / 1 / 2 / 3 сделаны** (сен 2026). Пункты **4–14** ещё в бэклоге                                                                                                                                                   |
| **Релиз**            | Текущая маркетинговая версия **1.3.2** (`pubspec` **1.3.2+1**). Feature-line **3** = пункты плана **1–3**; патч **2** = фикс чёрного экрана после 2FA. Следующий крупный блок → **1.4.0**; финал → **2.0.0**. Для пользователя / Settings / docs — только три числа, без рекламы `+build`. См. [TECHNICAL § Версионирование](TECHNICAL.ru.md#версионирование). |
...
| Почта                    | Входящие + страницы по 20 + счётчик непрочитанных + пометка прочитанным + перевод HU→EN/RU. **Нет поиска. Нет фильтра «только непрочитанные».** `filterType=0` зашит.                                                                                                                                |
...
| Settings | … существующие переключатели; **Contacts** + маркетинговая версия (`1.3.2`, без `+build`) внизу |
...
| --- | ---------------------------- | ----------------------------------------------------------------------------------- |
| 4   | Поиск и фильтр непрочитанных | Нужна живая сессия и стабильные `MailEntry.ID`. «Что изменилось» сравнивает эти id. |
...
### 4. Поиск писем + фильтр непрочитанных
...
- **Готово когда / Done when**  
Часть имени отправителя прячет остальные строки. Фильтр unread оставляет только 📬. Офлайн + кэш фильтрует локально.
- **Не делать / Out of scope**  
...
### 7. Точность `totalMoney` + антиспам нотификаций оплаты

- **Зачем / Why**  
Шапка: «потратил %0 Huf». Код суммирует только `completed`, `ammount.abs()`, из **50** последних транзакций. Стипендии (+ ) и взносы (− ) в «потратил» попадают одинаково, если статус completed. Неоплаченные (`aktív`) в сумму не входят, но могут породить **по нотификации на каждый оставшийся день** (или **32**, если `dueDateMs == 0`).
- **Зависит от / Depends on**  
...
- **Готово когда / Done when**  
Число в шапке совпадает с суммой **оплаченных взносов** в списке (или подпись честно говорит «нетто / 50 последних»). Включённые нотификации оплаты — **≤ 1 в день**, не десятки.
- **Не делать / Out of scope**  
