...
| **Релиз**            | Текущая маркетинговая версия **1.3.1** (`pubspec` **1.3.1+1**). Feature-line **3** = пункты плана **1–3**; патч **1** = auth / 2FA / messaging Student-web-full. Следующий крупный блок → **1.4.0**; финал → **2.0.0**. Для пользователя / Settings / docs — только три числа, без рекламы `+build`. См. [TECHNICAL § Версионирование](TECHNICAL.ru.md#версионирование). |
