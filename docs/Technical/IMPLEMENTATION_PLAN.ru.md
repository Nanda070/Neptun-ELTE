...
| ICS                      | **Импорт** + **экспорт** share (`ICSCalendar.shareIcsExport` из текущих `calendarEntries`). Кнопки импорта на хабе **нет**. Официальный webcal URL Neptun не подключён.                                                                                                                                                                                   |
...
| №   | Работа                                                            | Гейт                                             |
| --- | ----------------------------------------------------------------- | ------------------------------------------------ |
| 5   | Ghost / what-if                                                   | **СДЕЛАНО** (сен 2026). Живые átlag+/30 в popup; опциональная цель; та же формула `MarkbookMath`. |
| 6   | Сегодня + полоса ZH/дедлайны + ICS **export** + гранулярность пар | **СДЕЛАНО** (сент. 2026). После п. 3 |
...
- **Сделано / Shipped**  
Живой preview átlag + /30; опциональная цель через `MarkbookMath.minGradeForTargetAvg`; `GhostGradePopupData` + сброс. Версию маркетинга на этом пункте не поднимали.

---



### 6. Строка «сегодня» + полоса ZH/дедлайны + ICS export + гранулярность пар — **СДЕЛАНО**
...
- **Сделано / Shipped**  
Строка «сегодня» (`calendar_today_NextClass` / `NoClass`); полоса ZH/дедлайны (`isTask` + экзамены); ICS export через share (`ICSCalendar.shareIcsExport`); гранулярность `SETTING_ClassNotif10` / `5` / `0` (по умолчанию все вкл.); без пересборки class-notif при `SessionGuard.isAuthBlocked`.
...
### 9. «Что изменилось» (простое) — **СДЕЛАНО**
...
- **Сделано / Shipped**  
`SNAPSHOT_MailIds` / `SNAPSHOT_GradeKeys` (+ ready-флаги); pending `WHATSCHANGED_*`; чипы на полосе календаря + блок в drawer; тап → unread-почта / зачётка; первая сетевая отрисовка пишет baseline без баннера.
...
**Сделано (п. 12):** `StudentCardRequest` + `StudentCardPage` (drawer + Settings) — флаги банка, статус заявки, опциональный профиль/контакты; кэш `STUDENT_CardCacheJson` (без секретов). **QR UI нет.**
...
- **Готово когда / Done when**  
UI показывает только снятые поля и совпадает с Saját adatok / **заявкой** на карту / банком у аккаунта захвата. Декоративного QR нет. **Выполнено** — честно: wallet QR / номер / срок на HWEB **по-прежнему нет**; приложение их не выдумывает.
- **Не делать / Out of scope**  
NFC-эмуляция. Декоративная «карта» из аватара. Редактирование / POST личных или банковских данных (save XHR в этом захвате не кликали). Wallet QR (на вебе всё ещё нет).
...
| ICS         | `lib/API/ics_calendar.dart`                                                 | импорт + экспорт (`shareIcsExport`)                                           |
