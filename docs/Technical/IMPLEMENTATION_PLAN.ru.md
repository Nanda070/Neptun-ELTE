...
| **Релиз**            | Этот foundation + nav IA + polish-пакет отгружается как продукт **1.1** (`pubspec` **1.1.0+19**). Опубликованная базовая линия на GitHub — **1.0**, пока эту линию не затегают/не запушат. Патчи → **1.1.x**; следующие крупные фичи → **1.2+**. См. [TECHNICAL § Версионирование](TECHNICAL.ru.md#версионирование). |
