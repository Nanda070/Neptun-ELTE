...
**Status:** **v1 core** + **mail cache fix** + **calendar week UI** shipped in **1.5.6** (16 September 2026). **Background keep-alive** + **password retention** Settings toggles shipped in **1.5.7** (both default OFF). Calendar week chrome polish **1.5.8**; battery-minimized background cadence **1.5.9**; keep-alive reliability (drop device-idle, immediate resume refresh, password kept on manual logout) **1.5.10**. Portal/HWEB activity, cold-start proactive `GetNewTokens`, JWT `exp` parse, and live-test matrix remain **future / research**.  
**Owner:** Nanda.  
