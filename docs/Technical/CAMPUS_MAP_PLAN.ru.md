...
**Статус:** **Фазы 0–6 готовы** — фаза A **карта закончена** для MVP (пакет LD + LE + [QA_REPORT.md](campus_map_package/QA_REPORT.md)). **Фаза B (Flutter) MVP отгружена в 1.6.0**. Разрешение на JPG basemap всё ещё **pending** (не заявлять App Store redistribution; sideload/dev OK с honesty-баннером).  
**Владелец:** Nanda.  
