...
**Статус:** **ядро v1** + **фикс кэша почты** + **UI недели календаря** отгружены в **1.5.6** (16 сентября 2026). **Фоновый keep-alive** + **сохранение пароля** в Настройках отгружены в **1.5.7** (оба default OFF). Polish хрома недели календаря **1.5.8**; щадящий для батареи фон **1.5.9**; надёжность keep-alive (снят device-idle, сразу refresh при resume, пароль сохраняется при ручном logout) **1.5.10**. Portal/HWEB, проактивный `GetNewTokens` на cold start, парсинг JWT `exp` и live-test matrix — **будущее / исследование**.  
**Владелец:** Nanda.  
...
|-----------|------------------|
| `resumed` | **Сразу** `GetNewTokens` (**1.5.10**) + периодический maintenance каждые 3м30; refresh календаря/почты |
| `inactive` | Оставить foreground timer (краткий OS overlay) |
| `paused`, `detached`, `hidden` | Пауза / отмена timer; re-arm опционального фонового keep-alive |
...
- **По умолчанию (ядро v1):** **нет** периодических запросов, пока isolate Flutter не работает. Действует только foreground **3 мин 30 с** `GetNewTokens` (см. выше).
...
|-----------|----------|---------|
| **Android** | [`workmanager`](https://pub.dev/packages/workmanager) | Период **45 мин** (надёжность **1.5.10**; пол ОС всё ещё 15 мин); сеть + не низкий заряд; **без** `requiresDeviceIdle` (idle почти блокировал запуски в **1.5.9**); initial delay **15 мин**; **без** обязательной зарядки; Doze/OEM могут откладывать дальше. |
| **iOS** | [`background_fetch`](https://pub.dev/packages/background_fetch) | `UIBackgroundModes` = `fetch`; минимум **45 мин**; система может отложить или **никогда** не запустить (force-quit / выкл. Background Refresh). |

**Ограничение — батарея / надёжность (1.5.9 → 1.5.10):** предпочитать **более длинные** фоновые интервалы (**45 мин**) вместо пола ОС 15 мин — меньше гарантии refresh в фоне, меньше расход. **1.5.10** снимает `requiresDeviceIdle` (слишком строго) и ставит Android initial delay **15 мин**. Отменять WorkManager / BGFetch в `resumed` (foreground владеет maintenance; **сразу** GetNewTokens при resume). Coalesce: skip фонового тика, если последний успешный `GetNewTokens` (fg или bg) был в течение **25 мин**. **Без** обязательной зарядки. Честно: **ОС может откладывать или пропускать** задачи; фоновый maintenance — **best-effort**, не SLA.
...
- Уважать lock refresh (`_isRefreshingToken`); пропускать тик при foreground refresh или если успех был в окне coalesce (**25 мин**, **1.5.9**).
...
- Регистрировать OS-задачи только при toggle **вкл** + login + **не** `resumed`; отмена при resume / logout / toggle off.
...
### Закрыто (отгружено) vs бэклог

| Тема | Статус |
|------|--------|
| Интервал + выбор плагина (батарея) | **Отгружено** — WorkManager / `background_fetch`, **45 мин**, default **выкл** (**1.5.7**…**1.5.10**) |
| Фоновый refresh по JWT `exp` | **Бэклог** — клиент **не** парсит `exp` |
| Portal / HWEB activity | **Бэклог** — только исследование (раздел ниже) |
| Live-test matrix | **Бэклог** |
...
**Статус:** **возможный будущий подход**, **на одном уровне с** (не заменяя) foreground **3 мин 30 с** `GetNewTokens`. **Без реализации** в текущей волне; **исследование + опционально**, **ниже приоритет**, чем проактивный JWT refresh.
...
- **Вкл:** хранить пароль в secure storage (`neptun_password`) после конца сессии **и** при **ручном logout** (**1.5.10** продуктовый выбор — pre-fill после Log out; wipe только при выкл. toggle или `dataWipe`).
...
| **Ручной logout** | **1.5.10:** сохранять пароль при toggle ВКЛ (pre-fill); JWT / HasLogin всегда стирать. Toggle ВЫКЛ или `dataWipe` очищает пароль. |
...
| **Rate limits** | ELTE / hallgato могут ограничивать частые `GetNewTokens`; интервал **3 мин 30 с** — компромисс; в тестах смотреть 429 / ошибки. |
...
**Ядро v1** = foreground **3 мин 30 с** `GetNewTokens` + снятие wall-clock + cold start по токенам. **Не** требовать для ядра v1:
...
12. **Выбор фонового плагина** — **готово (1.5.7)**; **тюнинг батареи (1.5.9)** / **надёжность (1.5.10)** — Android `workmanager` **45 мин** + battery-not-low + сеть (idle снят); iOS `background_fetch` **45+ мин**; TECHNICAL § Session recovery.
13. **Политика батареи / ELTE** — **готово (1.5.7 / 1.5.9 / 1.5.10)** — более длинный фон; общий `GetNewTokens` + mutex; отмена BG в `resumed`; сразу refresh при resume; coalesce 25 мин; без зарядки; без device-idle.
14. **Настройки — сохранение пароля** — **готово (1.5.7)**; **сохранение при ручном logout (1.5.10)** — toggle (`SETTING_RememberPasswordOnDevice`, default **выкл**); пароль при ручном logout если ВКЛ; pre-fill входа; строки EN/HU/RU.
...
**Отгруженный фикс:** layout `WeekoffseterElementWidget` + строки `calendarPage_weekNav_*` (`lib/TimetableElements/timetable_element_widget.dart`, `lib/language.dart`, RU/TR JSON); заголовок education week и диапазон дат в подписи. **1.5.8** — единая карточка + фикс EN диапазона `${to.day}` (`lib/API/api_coms.dart`).
