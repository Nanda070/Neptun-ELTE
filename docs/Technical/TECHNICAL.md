...
Indoor campus map (LD/LE A→B): **map data first** — [CAMPUS_MAP_PLAN.md](CAMPUS_MAP_PLAN.md) / [RU](CAMPUS_MAP_PLAN.ru.md); Phase **0–4 done** (LD + LE MVP graphs + joins/aliases under [`campus_map_research/`](campus_map_research/)); Flutter UI deferred until the graph package is finished. Research: [`campus_map_research/`](campus_map_research/README.md).  
...
| Campus indoor map | **Research + LD/LE graph MVP + joins** | Dump + schema + graphs + [`joins/`](campus_map_research/joins/); [CAMPUS_MAP_PLAN](CAMPUS_MAP_PLAN.md) Phase 0–4 done, Phase 5 (package) next; Flutter UI deferred (Phase B). Shipped app still external maps deep-link only |
...
| `docs/Technical/CAMPUS_MAP_PLAN.md` / `.ru.md` | Finish-the-map-first plan — Phase 0–4 done (LD + LE MVP graphs + joins/aliases); Phase 5 package next; Phase B (Flutter) deferred; research + schema + graph + joins under `campus_map_research/` |
