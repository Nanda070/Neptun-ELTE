...
Indoor campus map (LD/LE A→B): [CAMPUS_MAP_PLAN.md](CAMPUS_MAP_PLAN.md) / [RU](CAMPUS_MAP_PLAN.ru.md). Phase A research package kept; Phase B **1.6.0** photo UX rejected; **1.7.0** graph-edge glow rejected; **1.7.1** mall-style floor schematic (shell + ribbons; graph routing/pins; JPG debug-only) + **IT faculty only** scope. Research: [`campus_map_research/`](campus_map_research/README.md).
...
- Version (`pubspec.yaml`): **1.7.2+1** — user-facing / Settings / docs = **1.7.2** (see [Versioning](#versioning) below).
...
Flutter still needs `x.y.z+build` in `pubspec.yaml` for stores. Prefer **`1.x.y+1`**. Use a larger `+N` only if Android requires a monotonic `versionCode`; never advertise `+N` as the product version. Settings shows **`info.version` only** (e.g. `1.7.2`). Mirror `build-name` to iOS `MARKETING_VERSION` / Android `versionName` fallbacks.
...
| **1.7.2** | **Current.** Patch — clean room labels (offsets/zoom/tap), floor-aware corridors, shorter IT-faculty chip (no tech banner / “for now”), docs scrub. Tag **v1.7.2**.
...
| `SettingsPage` (`settings_page.dart`) | Theme, language, font, notifications, haptics, week offset; optional **background keep-alive** + **Remember password on this device**; **Contacts** sheet + marketing version only (`package_info_plus` `info.version`, e.g. `1.7.2` — no `+build`) at bottom |
...
| Campus indoor map | **Strategy D schematic (1.7.1+)** | Package + `CampusMapPage`: LD/LE mall-style floor schematic (shell + corridor ribbons), room pins, smooth centerline route, collision-aware labels, **IT faculty only** banner. JPG debug-only. Graph for routing only. External Open map kept. |
