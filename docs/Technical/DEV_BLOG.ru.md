...
- **Уже в приложении (1.8.3):** полигоны BIS FootPrint; официальная светлая палитра этажа; LD+LE (факультет IK); Map до логина; поиск / этажи / карточка по тапу; оверлей A→B (**приблизительный** affine ≈WGS). Покрытие **2573/3112** MVT FootPrint (без catalog-bbox); hull underlay этажа.
- **Ещё WIP:** дыры / отсутствующие кольца (особенно **technical** — в BIS MVT **0** technical FootPrint; ~399 комнат каталога пропущено); нет полной визуальной паритетности с официальным BIS; выравнивание маршрута всё ещё приблизительное; дальнейшая оцифровка / покрытие отложены до возобновления работы.
- Без bump маркетинговой версии для этой docs-паузы. Владелец **Nanda**.
...
- **Релиз 1.7.0** (`pubspec` **1.7.0+1**): feature-line **7** — **Strategy D product UX**. Карта кампуса по умолчанию — **2D-схема из графа** (`CampusSchematicPainter`: полосы коридоров, пины/лейблы комнат, маркеры лифт/лестница, Chaikin-маршрут) — **не** фото плана этажа. JPG-подложка только debug (выкл.). Centerline-граф LE тоже отгружен. Pre-login Map / этажи / поиск / A→B сохранены. Honesty: schematic MVP из campus-графа; официальный BIS FootPrint тогда ещё не был primary; `routing.route` всё ещё **null**; репо **private**. Docs EN+RU + CAMPUS_MAP_PLAN синхронизированы. GitHub Release **v1.7.0**. Владелец **Nanda**.
...
- **Релиз 1.6.0** (`pubspec` **1.6.0+1**): feature-line **6** — **фаза B** indoor-карты кампуса MVP. Кнопка **Карта кампуса** на login-хабе (без JWT), пункт в drawer, корпуса LD/LE, этажи + pan/zoom, поиск joins/aliases, A→B Dijkstra, honesty-баннер (приблизительный граф; JPG basemap path transitional at the time). Пакет в `assets/campus_map/`. Внешний Open map для LD/LE/LK сохранён. GitHub Release **v1.6.0**. Владелец **Nanda**.
...
Фаза A («сначала закончить данные карты, потом любой Flutter Map UI») закрыта для MVP. Короткие буллеты того же дня по фазам 0–6 остаются хронологическими крошками; **эта** запись — полный дневник: что сдано, где лежит, как доказан QA, что тогда честно ещё не было сделано, и что значила фаза B позже. Только docs/data на тот момент — **ещё без** Flutter-экранов карты, **без** bump версии. (Flutter Map UI позже в **1.6.0+**; FootPrint primary **1.8.x**, всё ещё WIP.) Владелец **Nanda**.
...
|-----:|---------------|-------------------|
| **0** | Заморозка инвентаря: публичные JPG+таблицы LD/LE, пределы дампа BIS, планировщик Északi только как UX-референс, **не ждать** полилинии BIS `routing.route` (null в research), JPG basemap historically noted (позже не ship-blocker: путь FootPrint/MVT) | [CAMPUS_MAP_PLAN](CAMPUS_MAP_PLAN.ru.md) фаза 0 · [campus_map_research/README](campus_map_research/README.md) |
...
3. **На тот момент нет Flutter indoor Map UI** — фаза **B** ещё впереди (позже **1.6.0+**). Product README тогда не обещал in-app indoor A→B; «Открыть карту» в расписании — внешние Maps (и позже — Campus Map WIP).
...
- Фаза 6 QA по [`campus_map_package/`](campus_map_package/): [`run_qa.py`](campus_map_package/run_qa.py) → [`qa_matrix.json`](campus_map_package/qa_matrix.json) + [`QA_REPORT.md`](campus_map_package/QA_REPORT.md). Итог **pass=41 / fail=0 / waive=2** (restricted/closed не на MVP-комнатах). Sign-off владельца: фаза A **карта закончена** для MVP. Flutter фаза B на тот момент ещё не начата. Только docs/data; без Dart / bump версии. Владелец **Nanda**. *(Полный дневник: запись выше.)*

## 2026-09-16 — docs: карта кампуса фаза 5 (пакет deliverable)

**[2026-09-16]**

- Фаза 5 ready-to-bundle пакет: [`campus_map_package/`](campus_map_package/) (`graph_ld/le`, joins, алиасы, стабильные `basemaps/{ld|le}/f*.jpg`, `manifest.json`, `checksums.sha256`, `ATTRIBUTION.md`, `check_package.py`). JPG basemap в пакете для research/QA (позже legacy/debug; product primary = FootPrint/MVT). Только docs/data; без Dart / bump версии. Владелец **Nanda**.
...
- Добавлены [CAMPUS_MAP_PLAN.md](CAMPUS_MAP_PLAN.md) / [`.ru.md`](CAMPUS_MAP_PLAN.ru.md): сначала пакет графа LD/LE + QA, **потом** Flutter UI карты (фаза B после 0–6; позже отгружена). Кросс-ссылки из `campus_map_research/README.md`, TECHNICAL EN+RU, `keep-docs-current.mdc`. Только docs; без Dart / bump версии. Владелец **Nanda**.
...
- Signed IPA / TestFlight / App Store / Play production — **не** текущая цель. CI: unsigned IPA + Android debug APK only (без analyze/test job).
