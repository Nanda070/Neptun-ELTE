...
- Версия (`pubspec.yaml`): **1.8.0+1** — для пользователя / Settings / docs = **1.8.0** (см. [Версионирование](#версионирование) ниже).
...
| **1.8.0** | **Текущая.** Feature-line **8** — карта кампуса: **полигоны BIS FootPrint** (заливки roomType; карточка по тапу; этаж/корпус). A→B поверх через affine basemapPx→WGS (приблизительно). В бандле `polygons_ld/le.json` (**2571** из каталога **3670**). Тег **v1.8.0**.
...
| Indoor-карта кампуса | **Полигоны BIS FootPrint (1.8.0)** | Пакет + `CampusMapPage`: цветные FootPrint комнат BIS; карточка по тапу; этаж/корпус; A→B линия графа на WGS (affine ≈). **2571** полигонов vs каталог **3670** (честный gap). Чип IK. Внешний Open map сохранён. |
