...
- Версия (`pubspec.yaml`): **1.8.1+1** — для пользователя / Settings / docs = **1.8.1** (см. [Версионирование](#версионирование) ниже).
...
| **1.8.1** | **Текущая.** Патч — фикс позиций полигонов: MVT Y ставил многие LD-комнаты ~300 м севернее корпуса Déli (два кластера + пустой разрыв). Кольца **привязаны** к centroid каталога BIS (форма без изменений); вид plan-align через `rotationAngle` ≈78.5°; uniform scale. Тег **v1.8.1**. |
...
| Indoor-карта кампуса | **Полигоны BIS FootPrint (1.8.1)** | Пакет + `CampusMapPage`: цветные FootPrint; reanchor к centroid каталога + ≈78.5° plan-rotation; карточка по тапу; этаж/корпус; A→B на WGS (affine ≈). **2571** vs **3670** каталог. Чип IK. Внешний Open map сохранён. |
