...
| Indoor-карта кампуса | **Research + MVP-графы LD/LE + joins** | Дамп + схема + графы + [`joins/`](campus_map_research/joins/); [CAMPUS_MAP_PLAN](CAMPUS_MAP_PLAN.ru.md) фазы 0–4 готовы, далее фаза 5 (пакет); Flutter UI отложен (фаза B). В приложении по-прежнему только внешний maps deep-link |
...
| `docs/Technical/CAMPUS_MAP_PLAN.md` / `.ru.md` | Indoor-карта кампуса — **сначала карта** (фазы 0–4 готовы: MVP-графы LD + LE + joins/алиасы; далее фаза 5 пакет; фаза B Flutter отложена); research + схема + граф + joins в `campus_map_research/` |
