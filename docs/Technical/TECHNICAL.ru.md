...
Indoor-карта кампуса (LD/LE A→B): [CAMPUS_MAP_PLAN.ru.md](CAMPUS_MAP_PLAN.ru.md) / [EN](CAMPUS_MAP_PLAN.md). **Всё ещё в разработке / WIP** — не готовый 1:1 официального BIS. Research фазы A сохранён; Phase B **1.6.0** фото-UX отвергнут; **1.7.x** schematic UX заменён. **1.8.0** = **полигоны BIS FootPrint**; **1.8.1** = reanchor к centroid каталога + plan-rotation; **1.8.2** = hull + bbox; **1.8.3** = denser z19 MVT, **без catalog-bbox**, официальная светлая палитра BIS, подписи в полигоне (**2573/3112**). Отгружено: Map до логина, LD+LE, приблизительный A→B. Ещё WIP: дыры / technical-кольца, выравнивание маршрута, дальнейшая полировка (**на паузе** 2026-09-16). Полигоны: [`campus_map_research/bis/polygons/`](campus_map_research/bis/polygons/README.md).  
...
| **1.8.3** | **Текущая.** Патч — официальный светлый вид BIS: denser z17–19 MVT (всё ещё **0** technical); **без catalog-bbox**; pale yellow / tan / blue-pink; подписи roomNumber в полигоне; hull floorPlate; **2573/3112**. Карта всё ещё **WIP** (не готовый BIS 1:1). Тег **v1.8.3**. |
...
| `SettingsPage` (`settings_page.dart`) | Тема, язык, шрифт, уведомления, хаптика, неделя; опционально **фоновый keep-alive** + **Запомнить пароль**; **Contacts** + только маркетинговая версия (`package_info_plus` `info.version`, напр. `1.7.1` — без `+build`) внизу |
...
| Indoor-карта кампуса | **WIP — BIS FootPrint (1.8.3)** | Всё ещё в разработке — **не** готовый 1:1 официального BIS. Отгружено: светлая палитра; reanchor + ≈78.5°; **hull этажа**; **только MVT** (**2573/3112**); подписи; карточка; приблизительный A→B; чип IK; Map до логина. Ещё WIP: дыры / technical (~399 skipped), выравнивание маршрута, полировка (**на паузе**). Внешний Open map сохранён. |
