...
Indoor-карта кампуса: [CAMPUS_MAP_PLAN.ru.md](CAMPUS_MAP_PLAN.ru.md) / [EN](CAMPUS_MAP_PLAN.md). **Primary UX (1.9.0):** встроенный **официальный BIS WebView** (`bis.elte.hu`) после входа **ELTE IIG / Caesar**; учётные данные опционально в `flutter_secure_storage`. Офлайн-пакет FootPrint — research / debug (меню **Offline polygon**). Ранее: research фазы A; **1.6.0** фото-UX отвергнут; **1.7.x** schematic заменён; **1.8.x** офлайн-полигоны (**2573/3112**) — оставлены как debug. Полигоны: [`campus_map_research/bis/polygons/`](campus_map_research/bis/polygons/README.md).  
...
- Версия (`pubspec.yaml`): **1.9.0+1** — для пользователя / Settings / docs = **1.9.0** (см. [Версионирование](#версионирование) ниже).
...
| **1.9.0** | **Текущая.** Feature-line **9** — карта кампуса: **официальный BIS WebView** + IdP ELTE IIG / Caesar (`username_iig` / `password_iig`); опциональное сохранение в secure storage; pre-login hub/drawer; офлайн FootPrint — debug. Тег **v1.9.0**. |
| **1.8.3** | Патч — официальный светлый вид BIS (офлайн-полигоны): denser z17–19 MVT; **без catalog-bbox**; pale yellow / tan / blue-pink; подписи; hull; **2573/3112**. Как primary заменён **1.9.0**. Тег **v1.8.3**. |
...
| **1.6.0** | Feature-line **6** — **фаза B** indoor-карты MVP: pre-login Map, LD/LE, этажи, поиск, A→B Dijkstra, пакет в `assets/campus_map/`. UX позже отвергнут (кривые пути + фото этажа). JPG basemap transitional at the time. Тег **v1.6.0**. |
...
| **1.5.11** | Патч — **фаза A (0–6)** indoor-карты кампуса: docs/data готовы (MVP-пакет + QA **41 pass / 0 fail / 2 waive**); синхронизация честности (на тот ship в приложении ещё нет indoor A→B; Flutter фаза B пришла позже). Бинарь той же продуктовой линии, что **1.5.10** (надёжность сессии) + тег для device/GitHub. Тег **v1.5.11**. |
...
| **1.6.x**, … | Патчи на линии **6**. Следующий крупный блок → **1.7.0**. Следующий крупный блок после **1.5.x** → **1.6.0** (или **2.0.0**, если это финальный/RC срез). |
...
**Автообновление Android с GitHub:** `AppUpdater` (`lib/Misc/auto_updater.dart`) предлагает обновление только если `tag_name` последнего Release **строго новее** установленного `versionName`. Чтобы фикс дошёл до sideload-пользователей — новая маркетинговая `1.x.y`, **новый** git-тег `v1.x.y` и GitHub Release с новым APK; **не** перезаливать APK на тот же тег в расчёте на автоустановку. Политика: `.cursor/rules/android-github-release-tags.mdc`. Чистые docs/chore коммиты без APK можно не бампить/не тегировать. **Честность (2026-09-16):** GitHub-репо **private**, пока стратегия/ассеты карты кампуса не устоялись — публичные sideload-пользователи **без** collaborator-доступа обычно **не** видят private Releases, поэтому in-app GitHub auto-update для них может перестать работать, пока репо снова не станет public или не появится другой канал дистрибуции.
...
| `SettingsPage` (`settings_page.dart`) | Тема, язык, шрифт, уведомления, хаптика, неделя; опционально **фоновый keep-alive** + **Запомнить пароль** + **Сохранять вход BIS IIG**; **Contacts** + только маркетинговая версия (`package_info_plus` `info.version`, напр. `1.9.0` — без `+build`) внизу |
...
**Cold start:** `isColdStartSessionUsable()` пускает на Home при `HasLogin` и непустом access token — **без** wall-clock stamp. Долгий фон без foreground refresh может потребовать полный вход, если refresh JWT истёк на сервере.
...
**Срок JWT:** access-токены короткоживущие (~10–15 мин **наблюдательно** в Neptun — клиент **не** парсит JWT `exp`). На foreground проактивный refresh продлевает access, пока приложение открыто; без рабочего refresh token провал проактивного или реактивного refresh форсирует logout. `trySilentReauth()` для ELTE пропускается.
...
| Modern JWT + refresh | **Solid** | GET+Bearer на назначенном `hallgatoN`; проактивный + реактивный `GetNewTokens`; JWT `exp` **не** парсится; без клиентского wall-clock (1.5.6) |
...
| Indoor-карта кампуса | **BIS WebView (1.9.0)** | Primary: официальный `bis.elte.hu` в WebView после входа ELTE IIG / Caesar; сохранение IIG user+pass в secure storage (opt-out в Настройках). Офлайн FootPrint (**1.8.x**, **2573/3112**) — debug. Внешний Open map сохранён. |
...
**WidgetKit / App Widget:** iOS extension `TodayClassesWidget` — `CFBundleVersion` / ShortVersion из build settings (`CURRENT_PROJECT_VERSION` / `MARKETING_VERSION`, в синхроне с маркетингом **1.7.1**). Пустой `CFBundleVersion` у appex ломает install на устройстве (`MissingBundleVersion`). Android `TodayClassesWidgetProvider` читает тот же JSON-снимок (без JWT).
...
| `docs/Technical/CAMPUS_MAP_PLAN.md` / `.ru.md` | План карты — research 0–6 сохранён; **1.8.0** полигоны BIS FootPrint; пакет [`campus_map_package/`](campus_map_package/) |
