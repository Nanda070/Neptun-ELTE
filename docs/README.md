...
- **Campus map (Strategy D schematic)** — login-hub button **without** Neptun login; LD/LE **mall-style 2D floor schematic** (shell + corridor ribbons; not graph-glow; JPG debug-only), floors, search, A→B; **IT faculty only (for now)** banner; external Open map for room codes kept
...
| Campus map (Strategy D schematic **1.7.1** / IT-only) | Plan [`CAMPUS_MAP_PLAN.md`](Technical/CAMPUS_MAP_PLAN.md) · [RU](Technical/CAMPUS_MAP_PLAN.ru.md) · package [`campus_map_package/`](Technical/campus_map_package/) ([QA](Technical/campus_map_package/QA_REPORT.md)) · research [`campus_map_research/`](Technical/campus_map_research/README.md). Mall-style floor schematic; graph for routing; IT faculty LD+LE scope. |
