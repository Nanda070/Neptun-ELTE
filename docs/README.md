...
- **Campus map (still in development)** — login-hub button **without** Neptun login; LD/LE **BIS FootPrint** room polygons (official light palette), floors, search, approximate A→B overlay, tap room card; IT faculty chip; honest **2573/3112** MVT coverage + hull underlay. **Not** a finished 1:1 official-BIS clone (holes / missing technical rings; route alignment approximate). Map UX polish paused; external Open map kept
- **Timetable** — week view (Mon–Sun only; no next-Monday bleed); **today summary** + ZH/deadline strip; ICS **export** share; class-notif granularity 10/5/0 min; same-day break chips localized; period banners only in the period strip; calendar filters in Settings; training switcher when multiple trainings exist; tap room codes `LD`/`LE`/`LK` to decode, **Open map** (always visible for LD/LE/LK) opens **external** Apple/Google Maps for Lágymányos buildings (in-app indoor A→B is Campus Map **WIP**; external Open map remains for building pins)
...
| Campus map (**WIP** / BIS FootPrint **1.8.3** / IT-only) | Plan [`CAMPUS_MAP_PLAN.md`](Technical/CAMPUS_MAP_PLAN.md) · [RU](Technical/CAMPUS_MAP_PLAN.ru.md) · package [`campus_map_package/`](Technical/campus_map_package/) · polygons research [`campus_map_research/bis/polygons/`](Technical/campus_map_research/bis/polygons/README.md). Still in development — not a finished BIS 1:1. Shipped: FootPrint fills + light palette + hull; approximate A→B; **2573/3112**; IT LD+LE. |
