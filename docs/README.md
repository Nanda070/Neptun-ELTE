...
| **Version** | **1.5.2** (marketing) — scheme `1.<feature-line>.<patch>`; feature line **5** = plan items **10** (semester comparison) + **14** (homescreen widgets). Patch **1.5.2** = Android parity (App Widget MVP + deep-link / maps queries / release APK path). **1.5.1** removed Calendar “Next 48 hours”. Item **11** (Academic Progress / tanterv) **dropped**. Previous **1.5.0** = line-5 ship; **1.4.0** = items **5–9** + **12–13**. Final product → **2.0.0**. Settings shows three numbers only (no `+build`). ([full policy](Technical/TECHNICAL.md#versioning)) |
