...
- **Homescreen widget (iOS)** — WidgetKit “Today’s classes” from **calendar cache only** (no JWT in the widget). Tap opens Calendar. Missing cache → open the app; stale day labeled. **Android widgets not shipped** (Glance deferred). Honest with the **10-minute** session (widget is an offline snapshot).
