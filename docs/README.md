...
- **Campus map (transitional)** — login-hub button **without** Neptun login; LD/LE floors, search, A→B (LD centerline paths in **1.6.1**); photo basemap not final (Strategy D); repo private; external Open map for room codes kept
...
| Campus map (transitional; Strategy D + private repo) | Plan [`CAMPUS_MAP_PLAN.md`](Technical/CAMPUS_MAP_PLAN.md) · [RU](Technical/CAMPUS_MAP_PLAN.ru.md) · package [`campus_map_package/`](Technical/campus_map_package/) ([QA](Technical/campus_map_package/QA_REPORT.md)) · research [`campus_map_research/`](Technical/campus_map_research/README.md). Phase B MVP UX rejected; LD centerline in **1.6.1**; seek official 2D; repo **private** (updater caveat). Basemap JPG permission **pending**. |
