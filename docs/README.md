...
- **Campus map (BIS FootPrint polygons)** — login-hub button **without** Neptun login; LD/LE **colored BIS room footprints** (not schematic ribbons / JPG), floors, search, A→B overlay, tap room card; IT faculty chip; honest **2972/3670** coverage (hull underlay; per-floor footer); external Open map kept
...
| Campus map (BIS FootPrint **1.8.2** / IT-only) | Plan [`CAMPUS_MAP_PLAN.md`](Technical/CAMPUS_MAP_PLAN.md) · [RU](Technical/CAMPUS_MAP_PLAN.ru.md) · package [`campus_map_package/`](Technical/campus_map_package/) · polygons research [`campus_map_research/bis/polygons/`](Technical/campus_map_research/bis/polygons/README.md). BIS room footprints; hull underlay; graph for A→B; **2972/3670** coverage; IT faculty LD+LE. |
