...
- **Session** — while the app is open, proactive `GetNewTokens` every **3 min 30 s** (foreground only); session ends on **manual logout** or **dead refresh** (not a fixed 10-minute wall clock). Failed refresh → force logout + re-login (keeps username; **keeps academic cache** — **1**; “from cache” banner when stale/offline). No silent ELTE portal re-auth. Same-process logout → valid-password re-login works without killing the app (**1a**).
