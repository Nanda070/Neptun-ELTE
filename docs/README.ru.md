...
- **Карта кампуса (переходная)** — кнопка на login-хабе **без** Neptun-логина; LD/LE, этажи, поиск, A→B (LD centerline в **1.6.1**); фото-basemap не финал (стратегия D); репо private; внешний Open map для кодов аудиторий сохранён
...
| Карта кампуса (переходная; стратегия D + private repo) | План [`CAMPUS_MAP_PLAN.ru.md`](Technical/CAMPUS_MAP_PLAN.ru.md) · [EN](Technical/CAMPUS_MAP_PLAN.md) · пакет [`campus_map_package/`](Technical/campus_map_package/) ([QA](Technical/campus_map_package/QA_REPORT.md)) · research [`campus_map_research/`](Technical/campus_map_research/README.md). UX Phase B MVP отвергнут; LD centerline в **1.6.1**; искать официальную 2D; репо **private** (caveat updater). Basemap JPG permission **pending**. |
