...
- **Сессия** — пока приложение открыто, проактивный `GetNewTokens` каждые **3 мин 30 с** (только foreground); выход — **ручной logout** или **мёртвый refresh** (не фиксированные 10 мин wall-clock). Провал refresh → принудительный выход + повторный вход (логин сохраняется; **учебный кэш** — **1**; баннер «из кэша»). Без тихого portal re-auth. Logout → повторный вход в том же процессе (**1a**).
