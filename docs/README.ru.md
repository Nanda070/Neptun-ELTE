...
| **Версия** | **1.5.0** (маркетинг) — схема `1.<feature-line>.<patch>`; feature-line **5** = пункты **10** (сравнение семестров) + **14** (iOS WidgetKit MVP; Android Glance отложен). П. **11** (Academic Progress / tanterv) **снят**. Предыдущая **1.4.0** = п. **5–9** + **12–13**. Финал → **2.0.0**. Settings показывает только три числа (без `+build`). ([полная политика](Technical/TECHNICAL.ru.md#версионирование)) |
...
- **Виджет на домашнем экране (iOS)** — WidgetKit «Пары сегодня» **только из кэша календаря** (без JWT в виджете). Тап открывает Календарь. Нет кэша → откройте приложение; устаревший день помечен. **Android-виджетов нет** (Glance отложен). Честно с **10-минутной** сессией (снимок офлайн).
