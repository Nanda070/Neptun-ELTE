...
- **Сессия** — истёкший JWT → принудительный выход + повторный вход (без тихого portal re-auth; refresh token если есть)
- **Профиль в drawer** — приветствие с полным именем из `UserInfo` + код Neptun; аватар — **инициалы** (фото API не подтверждён); без training ID под именем; переключатель обучения при нескольких training
