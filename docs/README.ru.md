...
| **Версия** | **1.5.4** (маркетинг) — схема `1.<feature-line>.<patch>`; feature-line **5** = пункты **10** (сравнение семестров) + **14** (виджеты). Патч **1.5.4** = надёжность 10-мин wall-clock сессии на Android + фикс двойных emoji в Bug report. **1.5.3** = новая иконка приложения (Android + iOS) + splash branding. **1.5.2** = паритет Android (App Widget MVP + deep-link / maps queries / путь release APK; фикс белого экрана OTP тоже в той линии). **1.5.1** убрал полосу «Следующие 48 часов». П. **11** (Academic Progress / tanterv) **снят**. Предыдущая **1.5.0** = отгрузка линии 5; **1.4.0** = п. **5–9** + **12–13**. Финал → **2.0.0**. Settings показывает только три числа (без `+build`). ([полная политика](Technical/TECHNICAL.ru.md#версионирование)) |
