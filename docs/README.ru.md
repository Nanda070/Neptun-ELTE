...
| **Версия** | **1.5.1** (маркетинг) — схема `1.<feature-line>.<patch>`; feature-line **5** = пункты **10** (сравнение семестров) + **14** (iOS WidgetKit MVP; Android Glance отложен). Патч **1.5.1** убирает полосу календаря «Следующие 48 часов». П. **11** (Academic Progress / tanterv) **снят**. Предыдущая **1.5.0** = отгрузка линии 5; **1.4.0** = п. **5–9** + **12–13**. Финал → **2.0.0**. Settings показывает только три числа (без `+build`). ([полная политика](Technical/TECHNICAL.ru.md#версионирование)) |
