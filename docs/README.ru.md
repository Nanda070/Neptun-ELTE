...
- **Карта кампуса (Strategy D schematic)** — кнопка на login-хабе **без** Neptun-логина; LD/LE **2D-схема** (не фото этажа), этажи, поиск, A→B; внешний Open map сохранён
...
| Карта кампуса (Strategy D schematic **1.7.1** / только IK) | План [`CAMPUS_MAP_PLAN.ru.md`](Technical/CAMPUS_MAP_PLAN.ru.md) · [EN](Technical/CAMPUS_MAP_PLAN.md) · пакет [`campus_map_package/`](Technical/campus_map_package/) ([QA](Technical/campus_map_package/QA_REPORT.md)) · research [`campus_map_research/`](Technical/campus_map_research/README.md). 2D-схема из графа отгружена; схема этажа; граф для маршрутов; охват IK LD+LE. |
