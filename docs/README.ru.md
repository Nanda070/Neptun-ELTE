...
| **Версия** | **1.3.3** (маркетинг) — схема `1.<feature-line>.<patch>`; линия **3** = пункты плана 1–3; патч **3** = фикс мгновенного «сессия истекла» после 2FA; следующий крупный блок → **1.4.0**; финал → **2.0.0**. Settings показывает только три числа (без `+build`). ([полная политика](Technical/TECHNICAL.ru.md#версионирование)) |
